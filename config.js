/**
 * ╔══════════════════════════════════════════════════════════════╗
 * ║   m view — Production Config                                 ║
 * ║   Generated for Render Live Deployment                        ║
 * ╚══════════════════════════════════════════════════════════════╝
 */

// Fixed production endpoint mapping directly to your live Render instance
const PRODUCTION_URL = "https://screen-connect-rtca.onrender.com";

window.MVIEW_SERVER_URL = PRODUCTION_URL;
window.SCREEN_CONNECT_SERVER_URL = PRODUCTION_URL;

// Supabase Production Credentials
window.MVIEW_SUPABASE_URL = "https://iacdzpcoftxxcoigopun.supabase.co";
window.MVIEW_SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImlhY2R6cGNvZnR4eGNvaWdvcHVuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY0MjA1NTUsImV4cCI6MjA5MTk5NjU1NX0.5Eo21XrLTWL3RyKmuvJPdaS-NssraDMyAxVMFy-F054";

// v15 Enterprise Config
window.SC = window.SC || {};
window.SC.SERVER_URL = PRODUCTION_URL;
window.SC.CONFIG = {
    SUPABASE_URL: window.MVIEW_SUPABASE_URL,
    SUPABASE_ANON_KEY: window.MVIEW_SUPABASE_ANON_KEY
};